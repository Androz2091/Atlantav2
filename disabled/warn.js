...
module.exports.run = async (message, args, bot, emotes, data) => {
    ...
    var nb_warns = cwarns.lenght;
    console.log(nb_warns)

    var isKick = (data.guild_data.warns_sanctions['kick']) ? data.guild_data.warns_sanctions['kick'] : false;
    var isBan = (data.guild_data.warns_sanctions['ban']) ? data.guild_data.warns_sanctions['ban'] : false;
    ...
    if(isKick <= nb_warns){
        ...
        message.channel.send(emotes[2]+' | **'+member.user.username+'** vient d\'être averti.');
        member.send(emotes[2]+' | Tu viens d\'être averti sur **'+message.guild.name+'** pour la raison suivante : **'+reason+'**');
    }
